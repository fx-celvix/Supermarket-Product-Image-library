    ...
    const handleDownload = async (e: React.MouseEvent) => {
        ...
        try {
            ...
            try {
                ...
                console.warn(`Proxy download failed with status ${response.status}. Falling back to client fetch.`);
            } catch (proxyError) {
                console.warn("Proxy download network error. Falling back to client fetch.", proxyError);
            }
            ...
            try {
                ...
                console.warn("Client direct fetch failed (likely CORS). Falling back to new tab.", clientError);
            }
            ...
            toast.error('Failed to download image. Please try manually.');
        } finally {
                    ...
                    <div className="relative w-full h-full transition-transform duration-500 group-hover:scale-105">
                        ...
                        {!isVisible ? (
                            ...
                            <div className="w-full h-full flex flex-col items-center justify-center bg-gradient-to-br from-slate-100 to-slate-200 dark:from-slate-700 dark:to-slate-800 rounded-xl">
                                ...
                                <span className="text-xs font-medium text-slate-400 dark:text-slate-500">Image Coming Soon</span>
                            </div>
                        )}
                    </div>

                    {/* Overlay Actions (Only show if NOT in selection mode AND has valid image) */}
                    {!isSelectionMode && hasValidImage && (
